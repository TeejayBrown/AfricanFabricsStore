class SearchController < ApplicationController
  ...
  private
    def find_products
      ...
      searched_items = searched_items.where("products.name like ?", "%#{params[:keywords]}%") if params[:keywords].present?
      searched_items = searched_items.where("product_categories.name like ?", "%#{params[:category]}%") if params[:category].present?
      searched_items
    end
end
